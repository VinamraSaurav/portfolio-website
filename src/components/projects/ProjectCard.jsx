...
const ProjectCard = ({data}) => {
    const {id,title, image, alt, repoLink, liveLink, description}=data;
  ...
  return (
    <div className='z-0'>
        <div className={`w-[320px] rounded-md border-2 border-color-2 p-3 relative z-0 h-[480px]`}>
            <div className='flex justify-center items-center '>
                <img src={image} className='w-full rounded-t-md aspect-video'  alt={alt} />
            </div>
            <div>
                <div className='text-white text-lg font-bold font-popins mt-3 mb-[7px] line-clamp-2 h-[53px] '>{title} </div>
                <div className={`text-white text-sm font-popins mb-3 h-[140px]  ${open?'overflow-y-scroll ':' line-clamp-6 '} `}>{description}</div>
            <div className='relative h-[60px]'>
              <div className='absolute bottom-14 right-0'>
                <div className='w-fit text-white/50 underline text-xs hover:cursor-pointer' onClick={handleClick}>{open?'see less':'see more'}</div>
                </div>  
            <div className='absolute bottom-0 left-0 right-2 z-0'>
